...
//	点击《前缀字符》：1、输入法发送:{GB100}{G1004？}
//    2、扫描器返回：{G1004/1/XXX}   XXX表示添加的前缀字符
//    3、输入法正确收到数据后弹出添加前缀字符对话框，并返回{GB200}，
//      添加或删除完前缀字符点击确认后返回：{G1004/1/YYY} {GB202} 
//YYY表示修改后的前缀字符。
...
String[] asciistr={"NUL","SOH","STX","ETX","EOT","ENQ","ACK","BEL","BS","HT","LF","VT","FF","CR","SO","SI","DEL",
	...
	int selectIndex =0;
	public MyCmdPrefix(String input, USBService context) {
		this.input = input;
...
//		EditText EditTextH = new EditText(context);
		...
		TextViewH.setText(MResource.getIdByName(context, "string","command_prefix_add"));
		views[2] = TextViewH;
		mylength =length;
	...
	public AlertDialog getAlertDialog(final USBService context) {
		...
		builder.setMessage(MResource.getIdByName(context, "string","command_prefix_name"))
				.setPositiveButton(MResource.getIdByName(context, "string","command_button_submit"), new DialogInterface.OnClickListener() {
					...
					String	string1 = Util.Checkboolean(myoggleButton
						...
						OutCmdStr = "{G1004" + "/"+ string1  +"/"+ string
